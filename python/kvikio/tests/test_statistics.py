...
def test_sampling_series_adds_up(a_file):
    path, nbytes = a_file
    buffer = np.empty(nbytes // 8, dtype="u8")

    with kvikio.SamplingMonitor(resolution_ms=2) as monitor:
        with kvikio.CuFile(path, "r") as f:
            for _ in range(4):
                f.read(buffer)
                time.sleep(0.005)
    ...
    assert monitor.stats()[1] == 0, "samples were dropped"
    assert len(samples["num_ops"]) == monitor.stats()[0]
    # Nothing is lost between the intervals and nothing is counted twice.
    assert sum(samples["num_ops"]) == 4
    assert sum(samples["bytes_transferred"]) == 4 * nbytes
    # The intervals abut, so the series has no holes in it.
    assert samples["start_unix_ns"][1:] == samples["end_unix_ns"][:-1]
    # Wall clock, so the series lines up with anything else that is timestamped.
    assert abs(time.time_ns() - samples["start_unix_ns"][0]) < 60e9


def test_sampling_takes_leave_the_monitor_empty(a_file):
    monitor = kvikio.SamplingMonitor(resolution_ms=2)
    time.sleep(0.02)
